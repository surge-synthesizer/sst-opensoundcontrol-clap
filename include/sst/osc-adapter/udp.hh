...
class SockAddr
{
  ...
  public:
    ...
    int getPort() const
    {
        char servname[512];
        int err =
            getnameinfo(&addr_.sa, sizeof addr_, 0, 0, servname, sizeof servname, NI_NUMERICSERV);
    ...
    std::string asString() const
    {
        ...
        if (addr().sa_family)
        {
            ...
            int err = getnameinfo(&addr_.sa, sizeof addr_, hostname, sizeof hostname, servname,
...
struct UdpSocket
{
    ...
    SockAddr remote_addr; /* initialised for connected sockets. Also updated for bound sockets after
    ...
    enum
    {
        ...
        OPTION_FORCE_IPV4 = 1,
        OPTION_FORCE_IPV6 = 2,
        OPTION_DEFAULT = OPTION_FORCE_IPV4 // according to liblo's README, using ipv6 sockets causes
                                           // issues with other non-ipv6 enabled osc software
    };
    ...
    */
    bool bindTo(int port, int options = OPTION_DEFAULT) { return openSocket("", port, options); }
    ...
    */
    bool receiveNextPacket(int timeout_ms = -1)
    {
        ...
        int nread =
            (int)recvfrom(handle, &buffer[0], (int)buffer.size(), 0, &remote_addr.addr(), &len);
        if (nread < 0)
        {
...
#else
            if (errno != EAGAIN && errno != EINTR && errno != EWOULDBLOCK && errno != ECONNRESET &&
                errno != ECONNREFUSED)
            {
    ...
    bool sendPacket(const void *ptr, size_t sz) { return sendPacketTo(ptr, sz, remote_addr); }
    ...
    bool sendPacketTo(const void *ptr, size_t sz, SockAddr &addr)
    {
        ...
        do
        {
            ...
            if (res == -1 && WSAGetLastError() == WSAEINTR)
                ...
                sent = res;
#else
            // if (res == -1) cerr << "sendto handle=" << handle << ", res:" << res << ", sz=" << sz
  ...
  private:
    ...
    bool openSocket(const std::string &hostname, const std::string &port, int options)
    {
        ...
        if (options == OPTION_FORCE_IPV4)
            ...
            hints.ai_family =
                AF_UNSPEC; /* Allow IPv4 or IPv6 -- in case of problem, try with AF_INET ...*/
        hints.ai_socktype = SOCK_DGRAM; /* Datagram socket */
        hints.ai_flags =
            (binding ? AI_PASSIVE : 0); /* AI_PASSIVE means socket address is intended for bind */

        int err = 0;

        err = getaddrinfo(binding ? 0 : hostname.c_str(), port.empty() ? 0 : port.c_str(), &hints,
                          &result);
        if (err != 0)
        {
