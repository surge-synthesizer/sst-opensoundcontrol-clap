...
/* the various types that we handle (OSC 1.0 specifies that INT32/FLOAT/STRING/BLOB are the bare
 * minimum) */
enum
{
    TYPE_TAG_TRUE = 'T',
...
class Message
{
  ...
  public:
    ...
    class ArgReader
    {
      ...
      public:
        ArgReader(const Message &m, ErrorCode e = OK_NO_ERROR)
            : msg(&m), err(msg->getErr()), arg_idx(0)
        {
        ...
        /** retrieve a string argument (no check performed on its content, so it may contain any
         * byte value except 0) */
    ...
    Message(const std::string &s, TimeTag tt = TimeTag::immediate())
        : time_tag(tt), address(s), err(OK_NO_ERROR)
    {
    }
    ...
    */
    ...
    {
        return ArgReader(*this, fullPatternMatch(address.c_str(), test.c_str()) ? OK_NO_ERROR
                                                                                : PATTERN_MISMATCH);
    }
    ...
    {
        return ArgReader(*this, partialPatternMatch(address.c_str(), test.c_str())
                                    ? OK_NO_ERROR
                                    : PATTERN_MISMATCH);
    }
    ...
    void buildFromRawData(const void *ptr, size_t sz)
    {
        ...
        const char *type_tags_end =
            (const char *)memchr(type_tags_beg, 0, storage.end() - type_tags_beg);
    ...
    void packMessage(Storage &s, bool write_size) const
    {
        ...
        if (storage.size())
            memcpy(s.getBytes(storage.size()), const_cast<Storage &>(storage).begin(),
                   storage.size());
    }
...
class PacketReader
{
  public:
    ...
    /** extract the next osc message from the packet. return 0 when all messages have been read, or
     * in case of error. */
    Message *popMessage()
    {
  ...
  public:
    ...
    PacketWriter &endBundle()
    {
        if (bundles.size())
        {
            ...
            {
                pod2bytes<uint32_t>(
                    0, storage.getBytes(4)); // the 'empty bundle' case, not very elegant
            }
            if (bundles.size() > 1)
    ...
    /** the error flag will be raised if an opened bundle is not closed, or if more than one message
       is inserted in the packet without a bundle */
